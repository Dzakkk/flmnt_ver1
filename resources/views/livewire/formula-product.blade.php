    ...
    @foreach($inputs as $key => $value)
        <div class="input-group mb-3" wire:key="{{ $key }}">
            <div class="col-md-6">
                ...
                <input name="persentase[]" type="text" id="persentase-{{ $key }}" wire:model="inputs.{{ $key }}" class="form-control">
            </div>
            <div class="col-md-6">
                <label class="form-label" for="kandungan-{{ $key }}">Kandungan:</label>
                <select name="FAI_code_barang[]" id="kandungan-{{ $key }}" class="form-control select2" wire:model="faicodeOptions.{{ $key }}">
            ...
            <div class="col-md-2">
                <button wire:click.prevent="removeInput({{ $key }})" class="btn btn-danger" type="button">Remove</button>
            </div>
        </div>
    @endforeach
